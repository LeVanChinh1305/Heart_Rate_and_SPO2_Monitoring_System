...
// Ngưỡng cảnh báo (Giả định giá trị nếu chưa định nghĩa toàn cục)
#define g_heart_rate_max_threshold  120.0f
#define spo2_min_threshold          94.0f
...
void max30102_processing_task(void *pvParameters)
{
    ...
    // Bộ đếm hỗ trợ giảm tần suất gửi tin khi lỗi/ngừng thở, tránh spam MQTT
    uint32_t non_valid_counter = 0; 

    ESP_LOGI(TAG, "Task xử lý MAX30102 bắt đầu hoạt động...");

    while (1) {
        // Chờ ngắt từ cảm biến trong 1 giây
        uint32_t is_notified = ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(1000));

        // =================================================================
        // KỊCH BẢN 1: KHÔNG ĐEO THIẾT BỊ (Timeout ngắt, cảm biến phẳng lặng)
        // =================================================================
        if (is_notified == 0) {
            ESP_LOGW(TAG, "Timeout ngắt! Phát hiện không đeo thiết bị. Đưa chỉ số về 0.");
            
            // Xóa cờ ngắt cũ để bảo vệ bus I2C
            ...
            // 1. Đưa thông số cấu trúc toàn cục về 0 để cập nhật lên màn hình LCD ST7735
            if (xSemaphoreTake(g_data_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
                g_health_data.heart_rate = 0.0f;
                g_health_data.spo2 = 0.0f;
                g_health_data.ppg_valid = false; // Đánh dấu false để LCD báo "Sensing..." hoặc "No User"
                xSemaphoreGive(g_data_mutex); 
            }
            
            // 2. Gửi dữ liệu tĩnh bằng 0 lên MQTT cho giao diện Web
            if (mqtt_is_connected()) {
                ...
                // Gửi kèm một giá trị sóng thô phẳng lặng bằng 0 lên đồ thị
                mqtt_publish_raw_single(0); 
            }
            continue; 
        }

        // --- ĐOẠN 1: Bảo vệ I2C khi đọc mẫu dữ liệu từ FIFO ---
        ...
        // --- ĐOẠN 2: Xử lý thuật toán ---
        if (bus_clear) {
            for (int i = 0; i < samples_read; i++) {
                
                // Gửi sóng thô liên tục (Nếu ngừng thở, sóng sẽ là đường thẳng tiệm cận DC cố định)
                if (mqtt_is_connected()) {
                    mqtt_publish_raw_single((int32_t)sample_buffer[i].ir);
                }

                // Đưa vào thuật toán xử lý phân tích nhịp tim
                if (ppg_algorithm_process_sample(sample_buffer[i].red, sample_buffer[i].ir, &ppg_result)) {
                    
                    if (ppg_result.valid) {
                        // Người dùng đang đo bình thường và có mạch đập tốt
                        non_valid_counter = 0; 
                        ...
                        // =================================================================
                        // KỊCH BẢN 2: ĐEO THIẾT BỊ NHƯNG NGỪNG THỞ / MẤT MẠCH ĐẬP (Flatline)
                        // =================================================================
                        non_valid_counter++;

                        // Thuật toán báo dữ liệu không hợp lệ liên tục trong khoảng 100 mẫu (~1 giây)
                        if (non_valid_counter >= 100) {
                            non_valid_counter = 0; 
                            
                            ESP_LOGE("HEALTH_ALERT", "CẢNH BÁO: Tín hiệu phẳng, phát hiện dừng tuần hoàn / ngừng thở!");

                            // Ép giá trị sinh hiệu về 0 ngay lập tức cho màn hình LCD
                            if (xSemaphoreTake(g_data_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
                                g_health_data.heart_rate = 0.0f;
                                g_health_data.spo2 = 0.0f;
                                g_health_data.ppg_valid = false; // Đưa về false để kích hoạt hiển thị 0 hoặc cảnh báo trên LCD
                                xSemaphoreGive(g_data_mutex); 
                            }

                            // Bắn số 0 định kỳ lên MQTT để giao diện Web React chuyển màu báo động nguy kịch
                            if (mqtt_is_connected()) {
                                float current_temp = 0.0f;
                                if (xSemaphoreTake(g_data_mutex, pdMS_TO_TICKS(5)) == pdTRUE) {
                                    current_temp = g_health_data.body_temp;
...
void mlx90614_temperature_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        // --- Bảo vệ I2C khi đọc cảm biến nhiệt nội bộ của MAX30102 ---
        if (xSemaphoreTake(i2c_bus_mutex, pdMS_TO_TICKS(50)) == pdTRUE) {
        ...
        // --- Bảo vệ I2C khi giao tiếp với MLX90614 ---
        if (args->mlx90614_dev != NULL) {
            ...
            if (err_ta == ESP_OK && err_to == ESP_OK) {
                ...
                // Cập nhật giá trị vào biến cấu trúc dùng chung g_health_data
                if (xSemaphoreTake(g_data_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
                    g_health_data.body_temp = body_temp_object;
                    xSemaphoreGive(g_data_mutex);
                }

                // --- GỬI DỮ LIỆU LÊN MQTT NGAY KHI CÓ NHIỆT ĐỘ MỚI ---
                if (mqtt_is_connected()) {
                    float current_heart_rate = 0.0f;
                    float current_spo2 = 0.0f;

                    // Lấy nhanh giá trị HR và SpO2 hiện tại để đóng gói gửi đi cùng nhiệt độ
                    if (xSemaphoreTake(g_data_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
                    ...
                    // Gửi toàn bộ dữ liệu tổng hợp lên MQTT Broker
                    mqtt_pulish_data(current_heart_rate, current_spo2, body_temp_object);
                    ESP_LOGI("TEMP_MONITOR", "Đã gửi cập nhật nhiệt độ mới lên MQTT.");
                }
            ...
            ESP_LOGW("TEMP_MONITOR", "Cảm biến MLX90614 chưa sẵn sàng kết nối phần cứng.");
        }
    }
}
...
void lcd_display_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (has_first_data) {
            ...
            st7735_draw_string(lcd_dev, 4, 50, str_buff, 0x07E0, 0x0000); 
            
            snprintf(str_buff, sizeof(str_buff), "SpO2 : %.1f %%  ", last_valid_spo2);
            st7735_draw_string(lcd_dev, 4, 75, str_buff, 0x07E0, 0x0000); 
        } else {
            st7735_draw_string(lcd_dev, 4, 50, "Pulse: Sensing...", 0xFA60, 0x0000); 
...
void app_main(void)
{
    ...
    // 1. Khởi tạo khẩn cấp i2c_bus_mutex ngay dòng đầu tiên
    i2c_bus_mutex = xSemaphoreCreateMutex();
    if (i2c_bus_mutex == NULL) {
        ESP_LOGE("MAIN_APP", "Không thể tạo Mutex I2C!");
        return;
    }
    ...
    // Khởi tạo MAX30102 có dùng Mutex bảo vệ
    ...
    // Khởi tạo MLX90614 có dùng Mutex bảo vệ
    ...
    if (mlx_ok) {
        ...
        ESP_LOGE(TAG, "Mạch phần cứng MLX90614 không phản hồi trên Bus I2C! Vẫn tiếp tục chạy hệ thống...");
        mlx90614_device_handle = NULL;
    }

    // Cấu hình Ngắt cho GPIO chân INT của MAX30102
    ...
    // Khởi chạy các Task xử lý dữ liệu
